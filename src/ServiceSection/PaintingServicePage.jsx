...
const PaintingServicePage = () => {
  ...
  const handleCheckout = () => {
    if (cartCount > 0) {
      navigate('/checkout', { state: { from: 'painting' } });
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center space-x-2 text-purple-600 mb-4 font-bold">
                <Palette size={20} />
                <span className="text-sm uppercase tracking-widest">Transform Your Space</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight leading-tight">
                Professional <span className="text-blue-600">Home Painting</span>
              </h1>
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500">
                <div className="flex items-center bg-orange-50 text-orange-700 px-3 py-1 rounded-full">
                  <Star size={14} className="fill-orange-500 mr-1" /> 4.8 (8k+ Reviews)
                </div>
                <div className="flex items-center text-green-600 font-bold">
                  <ShieldCheck size={18} className="mr-1" /> Dust-Free Process
                </div>
              </div>
            </div>
            ...
            <section>
              ...
              {loading ? (
                ...
                <div className="p-10 bg-white rounded-3xl text-center border-dashed border-2 text-gray-400 italic">
                  No painting packages found for {projectType}.
                </div>
              )}
            </section>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                {services[0]?.inclusions?.map((inclusion, index) => (
                  <div key={index} className="flex items-start space-x-4 bg-white/5 p-5 rounded-2xl border border-white/10">
                    <div className="bg-blue-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0">{index + 1}</div>
                    <p className="text-slate-200 font-bold text-sm leading-tight">{inclusion}</p>
                  </div>
                ))}
              </div>
            </section>
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-xl sticky top-24">
              ...
              {cartCount > 0 ? (
                ...
                <p className="text-gray-400 text-center py-6 text-sm">List is empty.</p>
              )}
            </div>
          </div>
        </div>

...
const RoServicePage = () => {
  ...
  const fetchRoServices = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BACKEND_URL}/api/auth/services?category=${serviceCategory}`);
      setServices(response.data);
    } catch (error) {
      console.error("RO data fetch error:", error);
    } finally {
      setLoading(false);
    }
  };
            ...
            {roBrands.map((brand) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full border text-sm font-semibold transition-all ${
                  selectedRoBrand === brand ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:border-blue-400'
                }`}
              >
                {brand}
              </button>

...
import {
  ...
  ChevronDown, Waves, Thermometer, ShieldAlert, Loader2, ShoppingCart, Trash2, X
} from 'lucide-react';
...
const RoServicePage = () => {
  ...
  const [serviceCategory, setServiceCategory] = useState('Routine Service');
  const [selectedRoBrand, setSelectedRoBrand] = useState('');
  // --- REDUX STATE ---
  ...
  useEffect(() => {
    dispatch(fetchServices(serviceCategory));
  }, [serviceCategory, dispatch]);

  const roBrands = ["Kent", "Aquaguard", "Pureit", "Livpure", "Blue Star", "Havells", "LG", "Other"];

  // --- CHECKOUT HANDLER ---
  const handleCheckout = () => {
    if (cartCount > 0) {
      navigate('/checkout', { state: { from: 'ro-service' } });
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center space-x-2 text-cyan-600 mb-4 font-bold">
                <Waves size={20} />
                <span className="text-sm uppercase tracking-widest text-slate-900">Pure Water, Healthy Life</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight leading-tight">
                Professional <span className="text-blue-600">RO Purifier</span> Service
              </h1>
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500">
                <div className="flex items-center bg-cyan-50 text-cyan-700 px-3 py-1 rounded-full">
                  <Star size={14} className="fill-cyan-500 mr-1" /> 4.9 (10k+ Reviews)
                </div>
                <div className="flex items-center text-green-600 font-bold">
                  <ShieldCheck size={18} className="mr-1" /> Genuine Filter Warranty
                </div>
              </div>
            </div>

            <div className="flex bg-gray-100 p-1.5 rounded-2xl w-fit shadow-inner border border-gray-200">
              {['Routine Service', 'Repair & Parts'].map((type) => (
                <button
                  key={type}
                  onClick={() => setServiceCategory(type)}
                  className={`px-8 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${serviceCategory === type
      ...
      <div className="bg-white py-8 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
            Select RO Brand <ChevronDown size={18} className="ml-1 text-blue-600" />
          </h2>
          <div className="flex flex-wrap gap-3">
            {roBrands.map((brand) => (
              <button
                key={brand}
                onClick={() => setSelectedRoBrand(brand)}
                className={`px-6 py-2.5 rounded-full border text-sm font-semibold transition-all ${selectedRoBrand === brand ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:border-blue-400'
                  }`}
              >
                {brand}
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            <section>
              <h2 className="text-2xl font-black text-slate-900 mb-6 uppercase tracking-tight">Available Packages</h2>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {services.map((pkg) => {
                    ...
                    return (
                      <div key={pkg._id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:border-cyan-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group relative overflow-hidden">
                        <div className="flex-1">
                          <h3 className="font-bold text-slate-900 text-lg group-hover:text-blue-600">{pkg.packageName}</h3>
                          <p className="text-gray-500 text-sm mb-3">{pkg.description || "Expert water purifier service with TDS check."}</p>
                          <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-gray-400 space-x-6">
                            ...
                            <span className="text-cyan-600 font-bold">Health Check Included</span>
                          </div>
                        </div>
                ...
                <div className="text-center py-10 bg-white rounded-3xl border border-dashed text-gray-400">
                  No RO services found in database for this category.
                </div>
              )}
            </section>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl">
              ...
              <h2 className="text-2xl font-black mb-8 relative z-10">RO Maintenance Checklist</h2>
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-[2rem] border border-blue-100 shadow-xl sticky top-24">
              ...
              {cartCount > 0 ? (
                ...
                <p className="text-gray-400 text-center py-6 text-sm italic">Your basket is empty.</p>
              )}
            </div>
          </div>
        </div>

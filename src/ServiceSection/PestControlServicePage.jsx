...
import {
  ...
  ChevronDown, Trash2, Bug, ShieldAlert, Leaf, Loader2, ShoppingCart, X
} from 'lucide-react';
...
const PestControlServicePage = () => {
  ...
  const [selectedPestType, setSelectedPestType] = useState('General Pest');
  ...
  useEffect(() => {
    dispatch(fetchServices(selectedPestType));
  }, [selectedPestType, dispatch]);

  const homeSizes = ["1 BHK", "2 BHK", "3 BHK", "4 BHK+", "Commercial/Villa"];

  // --- CHECKOUT HANDLER ---
  const handleCheckout = () => {
    if (cartCount > 0) {
      navigate('/checkout', { state: { from: 'pest-control' } });
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center space-x-2 text-emerald-600 mb-4 font-bold">
                <Leaf size={20} />
                <span className="text-sm uppercase tracking-widest text-slate-900">Safe & Eco-Friendly Protection</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight leading-tight">
                Professional <span className="text-blue-600">Pest Control</span>
              </h1>
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500">
                <div className="flex items-center bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full border border-emerald-100">
                  <Star size={14} className="fill-emerald-500 mr-1" /> 4.9 (12k+ Reviews)
                </div>
                <div className="flex items-center text-green-600 font-bold">
                  <ShieldCheck size={18} className="mr-1" /> Non-Toxic Chemicals
                </div>
              </div>
            </div>

            <div className="flex bg-gray-100 p-1.5 rounded-2xl w-fit shadow-inner border border-gray-200">
              {['General Pest', 'Specialized'].map((type) => (
                <button
                  key={type}
                  onClick={() => setSelectedPestType(type)}
                  className={`px-8 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${selectedPestType === type
      ...
      <div className="bg-white py-8 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center tracking-tight">
            Select Property Size <ChevronDown size={18} className="ml-1 text-blue-600" />
          </h2>
          <div className="flex flex-wrap gap-3">
            {homeSizes.map((size) => (
              <button
                ...
                className={`px-6 py-2 rounded-full border text-sm font-semibold transition-all ${selectedHomeSize === size
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            {/* Packages Section */}
            <section>
              <h2 className="text-2xl font-black text-slate-900 mb-6 uppercase tracking-tight">Expert Treatment Plans</h2>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {services.map((pkg) => {
                    ...
                    return (
                      <div key={pkg._id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:border-emerald-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group relative overflow-hidden">
                        <div className="flex-1">
                          <h3 className="font-bold text-slate-900 text-lg group-hover:text-emerald-600 transition-colors">{pkg.packageName}</h3>
                          <p className="text-gray-500 text-sm mb-3">{pkg.description || "Safe and effective pest elimination service."}</p>
                          <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-gray-400 space-x-6">
                            ...
                            <span className="text-emerald-600 font-bold">WHO Approved Chemicals</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between md:flex-col md:items-end gap-2 w-full md:w-auto">
                          ...
                          {isInCart ? (
                            ...
                            <button
                              ...
                              className={`px-10 py-3.5 rounded-xl font-bold text-sm transition shadow-lg active:scale-95 shadow-emerald-100 ${pkg.isServiceActive
                ...
                <div className="p-10 bg-white rounded-3xl text-center border-dashed border-2 text-gray-400 italic">
                  No treatments found for {selectedPestType}.
                </div>
              )}
            </section>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl">
              ...
              <h2 className="text-2xl font-black mb-8 relative z-10">Our 3-Point Safety Standard</h2>
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl z-50 md:hidden">
          <div className="bg-slate-900 text-white px-6 py-4 rounded-3xl shadow-2xl flex items-center justify-between border border-white/10">
            ...
            <button
              onClick={handleCheckout}
              className="bg-emerald-600 px-8 py-3 rounded-2xl font-black text-sm"
            >
              Checkout
            </button>
          </div>
